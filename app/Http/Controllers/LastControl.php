...
*/
    ...
    public function saveReq($id,Request $request){
      ...
      ServiceRequest::create(['id'=>$service_request_id,'client_id'=>$id,'services_id'=>$request->service,'desc_of_service'=>$request->servDesc,'date_start'=>Carbon::now(),'meetingPlace'=>$request->meeting,'meetingSchedule'=>$meetDt,'status'=>'active','isRead'=>'1','created_at'=>Carbon::now(),'updated_at'=>Carbon::now()]);
...
*/
    ...
    public function saveAddGuardReq($id,Request $request){
      ...
      $add_guard_reques_id = 'ADDGUARDREQ-'.AddGuardRequests::get()->count();
       AddGuardRequests::create(['id'=>$add_guard_reques_id,'client_id'=>$id,'establishments_id'=>$request->establishment_id,'no_guards'=>$request->no_guards,'shift_start'=>$shift_start,'shift_end'=>$shif_end,'date_needed'=>$request->date_needed,'status'=>'active','created_at'=>Carbon::now(),'updated_at'=>Carbon::now(),'contract'=>$request->contracts]);
       return redirect('Request-'.$id);
    }
