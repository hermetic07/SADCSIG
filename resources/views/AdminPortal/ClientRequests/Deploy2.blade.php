...
$(".sel").text( " Please select " + guardsReq + " guards to deploy to the client.");
    $('#servReqID').val(radioVal[1]);
    $('#numGuards').val(radioVal[2]);
    $('#addGuardReqID').val(radioVal[0]);
    ...
    <div class="container-fluid">
      <div class="row bg-title">
        <div class="col-lg-3 col-md-4 col-sm-4 col-xs-12">
          <h4 class="page-title">Additional Guard Requests</h4>
        </div>
                  ...
                  <div class="wizard-pane active" role="tabpanel">
                    <div class="row  el-element-overlay">
                      <table id="demo-foo-addrow" class="table table-bordered table-hover toggle-circle color-bordered-table warning-bordered-table" data-page-size="10">
                        ...
                        @endphp
                        <tbody>
                        @foreach($addGuardRequest as $addGuardRequests)
                          @if($addGuardRequests->status != "deleted")
                           @foreach($establishments as $establishment)
                            @if($establishment->id == $addGuardRequests->establishments_id)
                              ...
                              @endphp
                              @if($addGuardRequests->status == "done")
                                ...
                                <td>
                                 ...
                                 <td>
                                  @foreach($clients as $client)
                                    @if($client->id == $addGuardRequests->client_id)
                                      {{ $client->name }}
                                    @endif
                                  @endforeach
                                 </td>
                                 ...
                                 <td>
                                   {{ $addGuardRequests->created_at }}
                                 </td>
                                 <td>
                                   {{ $addGuardRequests->status }}
                                 </td>
                                  <div class="radio radio-info">
                                    <input type="radio" id="{{ $addGuardRequests->id }}" class="radioBtn" value="{{ $addGuardRequests->id }},{{ $establishmentID }},{{ $addGuardRequests->no_guards }}" disabled="true">
                                 ...
                                 <td>
                                   <button class="btn btn-info" data-target="#{{ $addGuardRequests->id }}" data-toggle="modal" type="button" data-toggle="modal" data-target=".bs-example-modal-lg"><i class="fa fa-list"></i> View details</button>
                                  <button class="btn btn-danger" onclick="fun_delete('{!!$addGuardRequests->id!!}')"><i class="fa fa-times"></i> </button>
                                 </td>
                               </tr>
                              ...
                              @else
                                <tr>
                                 ...
                                 <td>
                                  <div class="radio radio-info">
                                    <input type="radio" id="{{ $addGuardRequests->id }}" class="radioBtn" value="{{ $addGuardRequests->id }},{{ $establishmentID }},{{ $addGuardRequests->no_guards }}">
                                 ...
                                 <td>
                                   <button class="btn btn-info view" type="button" data-target="#modalview" data-toggle="modal" id="{{ $addGuardRequests->id }}" ><i class="fa fa-list"></i> View details</button>
                        ...
                        <div class="row  el-element-overlay">
                          ...
                          <br> 
                            ...
                            <table id="demo-foo-addrow" class="table table-bordered table-hover toggle-circle color-bordered-table warning-bordered-table" data-page-size="10">
                              ...
                              <input type="hidden" id="servReqID" name="servReqID" value="{{ $establishmentID }}">
                              <input type="hidden" id="numGuards" name="numGuards" value="{{ $addGuardRequests->no_guards }}">
                              <input type="hidden" name="avGuards" value="{{ $ctr }}">
                              <input type="hidden" id="addGuardReqID" name="addGuardReqID" value="{{ $addGuardRequests->id }}">
...
<script type="text/javascript">
  ...
  $('.view').on('click',function(e){
   ...
   $.ajax({
      type:'GET',
      url: '{{ route("addguardrequests.view") }}',
      data: {'id':id},
